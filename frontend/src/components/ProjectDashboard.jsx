...
function ProjectDashboard({ project: initialProject, user, role, onBack, onLogout, onProjectUpdated, onViewToolRegistry, onViewDashboard }) {
  ...
  function generateDisclosure() {
    const useCaseDescriptions = {
      ...
      'grading': 'student grading and assessment',
    ...
    let text = `DISCLOSURE OF TECHNOLOGY USE\n\n`;
    text += `Activity: ${project.name}\n`;
    text += `Date: ${date}\n\n`;

    // What was used
    if (toolNames.length > 0) {
      text += `Technology tools used: ${toolNames.join(', ')}.\n\n`;
    }

    // Purpose
    text += `These tools were used for ${useCase}`;
    if (project.description) {
      text += ` — specifically, ${project.description.toLowerCase().replace(/\.$/, '')}`;
    }
    text += `.\n\n`;

    // Compliance steps taken
    if (completedCps.length > 0) {
      text += `The following compliance steps were completed as part of this activity:\n`;
      completedCps.forEach(cp => {
        text += `- ${cp.label}`;
        if (cp.completedAt) {
          text += ` (${new Date(cp.completedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })})`;
        }
        text += `\n`;
      });
      text += `\n`;
    ...
    // Human oversight
    const oversightDecisions = decisions.filter(d =>
      ['human_review', 'human_override', 'grading_validation', 'bias_audit', 'grading_fairness'].includes(d.checkpoint)
    );
    if (oversightDecisions.length > 0) {
      text += `Human oversight measures:\n`;
      oversightDecisions.forEach(d => {
        text += `- ${d.description}\n`;
      });
      text += `\n`;
    }
    ...
    // Data handling
    const dataDecisions = decisions.filter(d =>
      ['data_deidentified', 'data_storage', 'ferpa_compliance', 'data_classification'].includes(d.checkpoint)
    );
    if (dataDecisions.length > 0) {
      text += `Data handling:\n`;
      dataDecisions.forEach(d => {
        text += `- ${d.description}\n`;
      });
      text += `\n`;
    }
    ...
    if (allFrameworks.length > 0) {
      text += `This activity was reviewed against the following regulatory frameworks: ${allFrameworks.join(', ')}.\n\n`;
    }
            ...
            if (!hasMultipleRoles) {
              return (
                <div className="pd-header-actions">
                  <button className="pd-disclosure-btn" onClick={generateDisclosure}>Disclosure</button>
                  <button className="pd-export-btn" onClick={() => generateReport('full')}>Export Report</button>
                </div>
              );
            }
            return (
